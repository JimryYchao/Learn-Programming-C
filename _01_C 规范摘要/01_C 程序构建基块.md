...
**源程序** 由若干个 “源文件” 或 “翻译单元” 经编译器翻译，并由链接器将生成的对象文件链接成可执行文件的程序员。**翻译单元** 包括具有外部链接的函数、类型定义和标识符声明。源程序是指令、杂注、声明、定义、语句块和函数的集合。

**预处理指令** 指示 C 预处理器在编译之前对程序文本执行特定操作。
...
标识符的 **可见性** 确定由其 **范围**（scope，作用域）决定。标识符仅在其范围内可见。`static` 声明标识符内部链接、`extern` 声明外部链接。

- **文件范围**：文件范围声明的标识符或类型说明符从声明点到文件范围结束之间可见。
- **函数范围**：标签和函数参数具有函数范围。
- **块范围**：块范围的局部标识符从其定义声明点到包含定义的块结尾可见。

范围和链接描述了标识符的可见性。存储期描述了标识符关联的对象的生命周期。对象的 **生命周期** 是程序执行过程中保证为其保留存储的值。**标识符的存储持续时间决定其生命周期**。具有 **全局存储期** 的类型将在程序执行期间始终存在。所有的函数都具有全局存储期。

一般在块范围或函数范围创建的对象具有 **局部存储期**，离开范围后对象将消失。可以显式声明 `static` 静态链接的局部变量，它具有全局存储期且在函数中一直有效。
...
- **动态分配存储期**：目标程序或操作系统运行阶段动态地为源程序中的变量和对象分配存储空间，动态存储分配包括 **栈式** 或 **堆** 两种分配方式。

>---
#### 1.3. 链接

在不同的范围内或在同一范围内多次声明的标识符可以通过创建 **链接** 的方式来引用同一标识符或函数。
...
编译器设置 **名称空间** 来区分用于各种项的标识符。每个名称空间中的名称都是单一定义的。

- **语句标签**：函数块的标签具有标签名称空间，一般与 `goto` 相关。语句标签不必与其他名称有所不同，只保证单个函数块中名称单一定义。
- **结构、联合和枚举说明符**：说明符名称必须不同于具有相同可见性的所有其他结构、枚举或联合的名称。
- **结构或联合的成员**：成员名称在结构或联合中是单一定义的。
...
- **typedef 名称**：*typedef* 名称不能用作同一范围内的标识符。
...
完整的对象类型具有对齐要求，这些要求对可以分配该类型对象的地址施加了限制。对齐方式是表示可以分配给定对象的连续地址之间的字节数。类型对它的每个对象都施加了对齐要求，可以使用 `alignas` 请求更为严格的对齐。

基本对齐是小于等于 `alignof(max_align_t)` 的有效对齐。所有对象的实现都应支持基本对齐。以下类型的对齐要求应为基本对齐：
...
文本编码是执行字符集字符到字符常量或字符串文本值的实现定义的映射，它应支持从所有基本执行字符集值到实现定义的编码的映射。它可能包含多字节字符序列。

宽文本编码是执行字符集的字符到 `wchar_t` 字符常量或 `wchar_t` 字符串文本值的实现定义的映射，如果实现未定义 `__STDC_MB_MIGHT_NEQ_WC__`，则映射应生成与所有基本执行字符集值的文本编码相同的值。一个或多个值可以映射到执行字符集的一个或多个值。
...
整数常量的类型是可以表示其值的相应列表中的一个。

| Suffix                     | description                                                     |
| :------------------------- | :-------------------------------------------------------------- |
| none                       | `signed` or `unsigned` of `int`,`long int`,`long long int`      |
| `u`,`U`                    | `unsigned` of `int`, `long int`, `long long int`                |
| `l`, `L`                   | `signed` or `unsigned` of `long int`,`long long int`            |
| `ul`, `uL`, `Ul`, `UL`     | `unsigned` of `long int`, `long long int`                       |
| `ll`, `LL`                 | `long long int`, `unsigned long long int`                       |
...
浮点常量也可以是 E-计数法的十进制数字序列或 p-计数法的十六进制数字序列。

浮点常量被转换为内部格式。浮点常数在执行时的转换不应引发异常条件或浮点异常。同一源形式的所有浮点常量应转换为具有相同值的相同内部格式。
...
字符常量可以是单字节或多字节字符的序列。UTF-8  `char8_t` 字符常量以 `u8` 为前缀，`wchar_t` 字符常量以 `L` 为前缀，UTF-16 `char16_t` 字符常量以 `u` 为前缀，UTF-32 `char32_t` 字符常量以 `U` 为前缀。`wchar_t`、`char16_t`、`char32_t` 字符常量称为宽字符常量。序列的元素是源字符集的任何成员；它们以实现定义的方式映射到执行字符集的成员。
...
`auto` 在局部范围声明自动存储期的变量。局部变量声明时不会自动初始化，直接使用是未定义行为。`static` 局部变量具有静态存储期，无法直接用自动存储期对象进行初始化。

在文件范围内可以和其他存储类说明符一起出现，`auto` 作为类型说明符占位并由编译器进行类型推断。
...
使用 `constexpr` 声明常量表达式，在编译时计算；声明的对象不能具有原子类型、可变修改类型或 `volatile` 或 `restrict` 限定修饰的类型。`constexpr` 表达式在声明时具有初始值设定项。

```c
struct s { void *p; };
constexpr struct s A = { nullptr };
constexpr struct s B = A;
/* 尽管表达式 A.p 不是一个空指针常量，而只是一个空指针，但 B 的初始化中唯一的显式初始化项是 A，而不是 A.p，
因此该初始化没有违反常量约束。 */
```

使用存储类说明符 `constexpr` 声明的对象在翻译时其值永久固定。如果尚不存在，则将 `const` 限定隐式添加到对象的类型中。声明的标识符被视为相应类型的常量表达式。

在块范围中声明的对象，使用 `constexpr` 声明的自动类型具有自动存储持续时间，标识符没有链接，并且对象的每个实例都有一个唯一的地址，可通过 `&`（如果未使用 `register` 说明符声明）获得（如果有）。文件范围声明的常量表达式默认具有内部链接，并且每个看到相同文本定义的翻译单元都实现具有不同地址的单独对象。
...
`extern` 声明对在其他位置定义的变量或函数的引用，包含定义时具有外部链接。从声明位置到剩余翻译单元可见。具有块范围的函数标识符声明不应具有除 `extern` 之外的显式存储类说明符。
...
`register` 限定变量的存储类为寄存器类型，访问速度比内存中的变量更快。当被编辑器忽略时，转换为自动类型。无法获取寄存器变量的地址。
...
`thread_local` 声明线程存储期的变量。块范围中声明时，必须是静态类型 `static` 或 `extern` 外部引用。线程对象在启动线程时开始初始化，每个线程拥有唯一副本。

>---
#### 4.7. typedef

`typedef`为现有类型声明别名，而不是创建新类型。`typedef` 名称与通常标识符（例如枚举项、变量或函数）共享命名空间。
