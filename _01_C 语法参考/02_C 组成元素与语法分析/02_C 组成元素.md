...
特性为各种源构造（如类型、对象、标识符或块）指定其他信息。它们由特性令牌标识，该令牌可以是特性前缀令牌（对于特定于实现的特性），也可以是由标识符指定的标准特性（C 标准中指定的特性）。标准中指定的任何标准特性的支持都是实现定义的且可选的。实现不支持的任何属性标记都将被忽略。

特性被认为属于某个源结构，由它们出现的语法上下文标识，对于每个单独的特性，相应的子句限制了该附属物在其中有效的语法上下文。附属于某个源构造的特性说明符序列应只包含允许应用于该源构造的特性。
...
特性令牌在特性列表中的显示顺序不重要，每个实现都应为特性前缀令牌中的特性前缀选择一个独特的名称。实现不应定义没有特性前缀的特性，除非它是 C 标准中指定的标准特性。

```c
// 假设实现选择特性前缀 hal 并提供名为 daisy 和 rosie 的特定特性。
[[deprecated, hal::daisy]] double nine1000(double);
[[deprecated]] [[hal::daisy]] double nine1000(double);
[[deprecated]] double nine1000 [[hal::daisy]] (double);
```

在语言的所有方面，指定为标识符 `attr` 的标准特性和形式为 `__attr__` 标识符在用作特性标记时的行为应相同，但拼写除外。特性 `[[nodiscard]]` 和 `[[__nodiscard__]]` 可以自由互换。
...
对于同一实现，以下两个声明是等效的，因为特性列表中的排序并不重要。
...
另一方面，以下两个声明并不等价，因为不同特性说明符的顺序可能会影响语义。
...
`nodiscard` 特性应用于函数、枚举、结构或联合类型的定义。
