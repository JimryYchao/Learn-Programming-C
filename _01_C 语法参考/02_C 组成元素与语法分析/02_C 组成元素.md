...
- [3. 存储类说明符](#3-存储类说明符)
	...
	- [3.5 \_Thread\_local](#35-_thread_local)
	- [3.6 试探性定义](#36-试探性定义)
- [4. 类型说明符](#4-类型说明符)
...
- [9. 属性说明符（C23）（未整理完成）](#9-属性说明符c23未整理完成)
	...
	- [9.3 C23 标准](#93-c23-标准)
		...
		- [9.3.2 fallthrough 贯穿抑制警告](#932-fallthrough-贯穿抑制警告)
		- [9.3.3 nodiscard 弃值表达式警告](#933-nodiscard-弃值表达式警告)
		- [9.3.4 maybe\_unused 抑制对未使用实体的警告](#934-maybe_unused-抑制对未使用实体的警告)
...
- 文件作用域的 ```restrict``` 限定指针必须在程序运行期间指向单个数组的元素
...
- ```sizeof``` 不能操作 BitDomain 的位字段，带有位字段的结构提供一种记录设置的方便途径，位字段的储存顺序自低位到高位方向
...
- “数组声明” 将命名数组并指定其元素的类型，其特征是元素的类型相同。带数组类型的变量被视为指向数组首元的指针
